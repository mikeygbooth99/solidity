...
ExpressionStatement makePopExpressionStatement(
...
OptionalStatements reduceNoCaseSwitch(Dialect const& _dialect, Switch& _switchStmt)
{
	...
	return make_vector<Statement>(makePopExpressionStatement(
		loc,
		_dialect.discardFunction()->name,
	...
	GenericFallbackReturnsVisitor<OptionalStatements, If, Switch> const visitor(
		[&](If& _ifStmt) -> OptionalStatements {
			if (_ifStmt.body.statements.empty() && m_dialect.discardFunction())
			{
				...
				s->emplace_back(makePopExpressionStatement(
					_ifStmt.location,
					m_dialect.discardFunction()->name,
